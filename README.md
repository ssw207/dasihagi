...
매번 다른 칸(제휴사명·금액)은 비워 두고 녹화하세요. 재생은 그 칸을 덮지 않고 **노랗게** 남겨 **나중에 입력**하라고 표시합니다. 편집에서 **손댈 칸**을 켜고 끌 수 있습니다. 상신·결제·전송 같은 위험 클릭은 재생이 **누르지 않고** `직접 누르기`로 안내하며, 파일 첨부 칸은 `직접 첨부`로 멈춥니다. **다시 찍기**로 깨진 칸 하나만 고칠 수 있습니다.
...
**⋯ → 편집**에서 이름, 사이트 패턴(한 줄에 하나), 칸의 표시 이름, **손댈 칸**, **재생 금지**(클릭), **프리셋별 재생 속도**를 고칩니다. 손댈 칸은 재생이 값을 넣지 않고, 재생 금지는 클릭을 누르지 않습니다. 깨진 칸은 **다시 찍기**로 그 칸만 고칩니다.
...
- 내보내기: JSON 파일에 **실제 값**이 들어갑니다. 파일을 안전하게 보관하세요. 팀에 공유할 땐 **값 없이 내보내기**로 구조만 보낼 수 있습니다(값은 비어 있고 손댈 칸·금지 표시는 유지).
...
사내 어드민이 **iframe** 안이어도 그 안에서 녹화·재생됩니다. 껍데기가 먼저 뜨고 안쪽 화면이 나중에 열려도 녹화가 이어집니다. 아주 작은 iframe에는 칩이 안 보일 수 있고, 그때는 바깥 페이지 칩에서 종료하면 됩니다.
...
| iframe에서 안 됨 | 확장 새로고침 후 다시 녹화. 칩이 없으면 바깥에서 종료. sandbox로 막힌 iframe은 불가 |
...
제품 범위·하지 않는 일은 [`기획서.md`](기획서.md)에 있습니다. 사내 테스트 한정 개발 기안은 [`docs/기안-사내테스트-다시하기.md`](docs/기안-사내테스트-다시하기.md)입니다 (2026-08-30 승인). 작업 메타프롬프트는 [`메타프롬프트/`](메타프롬프트/)에 있습니다.

사용에는 아래 명령이 필요 없습니다. 숫자는 현재 소스 스위트 기준입니다.

```bash
npm install
npm run check
npm run test:encrypt   # 13
npm run test:pii       # 30
npm run test:replay    # 161 (손댈 칸·위험 클릭·파일·프리셋 속도·리픽·스크럽 포함)
npm run test:qa        # 22
npm run test:e2e       # 71 (시나리오 A~L)
```

엣지케이스 목록은 [`QA-엣지케이스.md`](QA-엣지케이스.md)에 있습니다.

README 움짤을 다시 찍으려면 `npm run gif` 입니다.

에이전트 워크플로 **BMAD**는 짧게만 씁니다: `bmad-brainstorming`(아이디어) → `bmad-spec`(기획) → `bmad-build`(구현). 안내는 `bmad-help`. 제품 설치와 무관합니다.
